document.getElementById('calculate-btn').addEventListener('click', function () {
    ...
    const neighborhood = document.getElementById('neighborhood').value; // Get the selected neighborhood

    // Validate inputs
    if (isNaN(x) || isNaN(x2) || isNaN(x3) || !neighborhood) {
        alert("Please enter valid numbers for all inputs and select a neighborhood.");
        return;
    }
    ...
    numUnitsElem.innerHTML = `Number of Units: <span>${numberOfUnits}</span> (Neighborhood: ${neighborhood})`;
